...
export default function BodiesTable(props) {
    ...
    const handleScroll = () => {
        // console.log(100 - (Math.round((refText.current.scrollTop * 100) / refText.current.scrollHeight)));
        // const progressBarLenght = (refText.current.scrollTop * 100) / refText.current.scrollHeight;
        const height = 94 - (Math.round((refText.current.scrollTop * 100) / refText.current.scrollHeight));

        refProgressBar.current.style.bottom = `${height}%`;
        console.log(refProgressBar.current.style.bottom);
    }
