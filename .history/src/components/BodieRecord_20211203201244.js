...
export default function BodieRecord(props) {
    ...
    const handleRecordClick = () => {
        ...
        console.log(refInput.current.offsetTop);
        refInput.current.classList.toggle('active');
        refInput.current.style.marginTop = `${refInput.current.offsetTop}px`;
        refInput.current.parentNode.parentNode.style.overflow = 'hidden';
        if(!refInput.current.classList.contains('active')) {
            refInput.current.parentNode.parentNode.style.overflow = 'scroll';
        }
    };
