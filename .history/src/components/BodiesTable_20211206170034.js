...
export default function BodiesTable(props) {
    ...
    const handleScroll = () => {
        console.log(Math.round((refText.current.scrollTop * 100) / refText.current.scrollHeight));
        // const progressBarLenght = (refText.current.scrollTop * 100) / refText.current.scrollHeight;
        // const height = refText.current.scrollTop;

        // refProgressBar.current.style.height = `${height}px`;
    }
